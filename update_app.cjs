...
appCode = appCode.replace(processHistoricalCandleOld, processHistoricalCandleNew);
...
appCode = appCode.replace(wsProcessOld, wsProcessNew);
...
appCode = appCode.replace(testBtnOld, testBtnNew);

const resetBtnOld = `<button class="btn btn-gray" id="btn-save-scripts" style="width: 100%; margin-top: 8px;">Save Scripts</button>`;
const resetBtnNew = `<div style="display: flex; gap: 8px; margin-top: 8px;">
...
appCode = appCode.replace(resetBtnOld, resetBtnNew);
...
appCode = appCode.replace(cssOld, cssNew);
...
appCode = appCode.replace(maxOld, maxNew);
...
appCode = appCode.replace(ringOld, ringNew);
...
appCode = appCode.replace(karenScriptOld, karenScriptNew);
